import { MapPin, PlayCircle, ShieldCheck } from "lucide-react";

import { formatPrice } from "@/lib/format";
import type { CattleListing } from "@/models/cattle";

export function CattleCard({ listing }: { listing: CattleListing }) {
  return (
    <article className="overflow-hidden rounded-[1.75rem] border border-brand-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="space-y-4 p-5">
        ...
        <a
          href="#top"
          className="inline-flex h-11 w-full items-center justify-center rounded-2xl bg-ink-900 px-4 text-sm font-semibold text-white transition hover:bg-brand-700"
        >
          View Details
        </a>
      </div>
